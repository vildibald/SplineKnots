...
void splineknots::ReducedKnotsGenerator::RightSideCross(
    ...
    auto three_div_hx = pchx.deboor_precalculated_.three_div_h;
    auto three_div_hy = pchy.deboor_precalculated_.three_div_h;
    ...
    for (int k = 0, j = 4; k < equations_count - 1; k++, j += 2) {
        ...
        auto j_plus_2 = j + 2;
        auto j_minus_2 = j - 2;
        rightside[k] = one_div7 * (knots.Dxy(i_minus_2, j_plus_2) +
                                   knots.Dxy(i_minus_2, j_minus_2)) -
                       2 * knots.Dxy(i_minus_2, j)
                       + three_div7_hx * (knots.Dy(i_minus_2, j_plus_2) +
                                          knots.Dy(i_minus_2, j_minus_2)) +
                       three_div7_hy * (-knots.Dx(i_minus_2, j_plus_2) +
                                        knots.Dx(i_minus_2, j_minus_2))
                       + nine_div7_hx * (knots.Dy(i, j_plus_2) +
                                         knots.Dy(i, j_minus_2)) +
                       nine_div7_hxhy * (-knots.Z(i_minus_2, j_plus_2) +
                                         knots.Z(i_minus_2, j_minus_2))
                       + twelve_div7_hx * (-knots.Dy(i_minus_1, j_plus_2) -
                                           knots.Dy(i_minus_1, j_minus_2)) +
                       twelve_div7_hy * (knots.Dx(i_minus_2, j_plus_1) -
                                         knots.Dx(i_minus_2, j_minus_1))
                       + three_div_hy * (knots.Dx(i, j_plus_2) -
                                         knots.Dx(i, j_minus_2)) +
                       twentyseven_div7_hxhy * (-knots.Z(i, j_plus_2) +
                                                knots.Z(i, j_minus_2)) -
                       thirtysix_div7_hxhy * (
                               knots.Z(i_minus_1, j_plus_2) -
                               knots.Z(i_minus_1, j_minus_2) +
                               knots.Z(i_minus_2, j_plus_1) -
                               knots.Z(i_minus_2, j_minus_1))
                       - six_div_hx * knots.Dy(i_minus_2, j) + twelve_div_hy * (
                knots.Dx(i, j_plus_1) + knots.Dx(i, j_minus_1)) +
                       ...
                       (-knots.Z(i_minus_1, j_plus_1) +
                        knots.Z(i_minus_1, j_minus_1)) + twentyfour_div_hx * knots.Dy(
                i_minus_1, j);
    }
    rightside[0] -= dfirst;
    rightside[equations_count - 1] -= knots.Z(i_minus_1, columns - 3) - eta * dlast;
}

void splineknots::ReducedKnotsGenerator::FillXDerivations(KnotMatrix &
values) {
    double three_div_4h = 0.25 * precalculated_hx_.deboor_precalculated_.three_div_h;
    ...
    utils::For(0, ncols,
               [&](int j) {
                   ...
                   for (size_t i = 1; i < nrows - 1; i += 2) {
                       values.SetDx(i, j,
                                    three_div_4h * (values.Z(i + 1, j) - values.Z(i - 1, j))
                                    - 0.25 * (values.Dx(i + 1, j) + values.Dx(i - 1, j))
                       );
                   }
               },
               1, is_parallel_);
...
values) {
    auto three_div_4h = precalculated_hy_.deboor_precalculated_.three_div_h;
    int ncols = values.ColumnsCount();
    int nrows = values.RowsCount();
    utils::For(0, nrows,
               [&](int i) {
                   ...
                   for (size_t j = 1; j < ncols - 1; j += 2) {
                       values.SetDy(i, j,
                                    three_div_4h * (values.Z(i, j + 1) - values.Z(i, j - 1)) -
                                    0.25 * (values.Dy(i, j + 1) + values.Dy(i, j - 1))
                       );
                   }
               },
               1, is_parallel_);
