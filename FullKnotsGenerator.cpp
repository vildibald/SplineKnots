...
namespace splineknots {
    Tridiagonals &FullKnotsGenerator::Tridagonals() {
        return tridagonals_;
    }

    splineknots::Tridiagonal &FullKnotsGenerator::Tridiagonal() {
        return tridagonals_[omp_get_thread_num()];
    }

    FullKnotsGenerator::Precalculated::Precalculated(const double h)
            : h(h), three_div_h(3 / h) {
    }

    FullKnotsGenerator::
    FullKnotsGenerator(MathFunction math_function, bool buffered) :
    ...
    FullKnotsGenerator::FullKnotsGenerator(
    ...
    FullKnotsGenerator::FullKnotsGenerator(MathFunction math_function,
    ...
    FullKnotsGenerator::FullKnotsGenerator(InterpolativeMathFunction
    ...
    KnotMatrix FullKnotsGenerator::GenerateKnots(const SurfaceDimension &
    ...
    void FullKnotsGenerator::Precalculate(const SurfaceDimension &udimension,
    ...
    void FullKnotsGenerator::InitializeKnots(const SurfaceDimension &
    ...
    void FullKnotsGenerator::FillXDerivations(KnotMatrix &values) {
    ...
    void FullKnotsGenerator::FillXYDerivations(KnotMatrix &values) {
        FillXYDerivations(0, values);
        FillXYDerivations(values.ColumnsCount() - 1, values);
    }

    void FullKnotsGenerator::FillYDerivations(KnotMatrix &values) {
    ...
    void FullKnotsGenerator::FillYXDerivations(KnotMatrix &values) {
        utils::For(0, static_cast<int>(values.RowsCount()),
                   [&](int i) {
    ...
    void FullKnotsGenerator::FillXDerivations(const int column_index,
                                              KnotMatrix &values) {
    ...
    void FullKnotsGenerator::FillXYDerivations(const int column_index,
                                               KnotMatrix &values) {
        auto unknowns_count = values.RowsCount() - 2;
        if (unknowns_count == 0) return;
    ...
    void FullKnotsGenerator::FillYDerivations(const int row_index,
                                              KnotMatrix &values) {
    ...
    void FullKnotsGenerator::FillYXDerivations(const int row_index,
                                               KnotMatrix &values) {
        auto unknowns_count = values.ColumnsCount() - 2;
        if (unknowns_count == 0) return;
    ...
    bool FullKnotsGenerator::IsParallel() {
        return is_parallel_;
    }

    const FullKnotsGenerator::Precalculated &
    FullKnotsGenerator::
    ...
    void
    FullKnotsGenerator::SetPrecalculatedHX(
            Precalculated precalculated) {
        precalculated_hx_ = std::move(precalculated);
    }

    void FullKnotsGenerator::SetPrecalculatedHY(
            Precalculated precalculated) {
        precalculated_hy_ = std::move(precalculated);
    }


    void FullKnotsGenerator::InParallel(bool value) {
        is_parallel_ = value;
    ...
    void FullKnotsGenerator::InitializeBuffers(const size_t u_count,
